...
class Data_Encoder_FAST(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print("index:type:",self.df[self.df.HADM_ID == index].DOC_EMB.values,type(self.df[self.df.HADM_ID == index].DOC_EMB.values))

        a= self.df[self.df.HADM_ID == index].DOC_EMB.values
        for n,i in enumerate(a):
            ...
            a[n]= i
        #print("a",type(a))
        #cat1=torch.from_numpy(self.df[self.df.HADM_ID == index].DOC_EMB.values)
        #doc_seqs = torch.cat(list(self.df[self.df.HADM_ID == index].DOC_EMB.values), 0)
        doc_seqs = torch.cat(tuple(a), 0)
